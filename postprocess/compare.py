...
def main():
    ...
    for mode in ["plain", "postprocessed"]:
        ref_gdf = gpd.read_file(f"{ROOT}/validation/polygons_{mode}_{name}.geojson", crs=f"EPSG:{crs}")
        comp_gdf = gpd.read_file(f"{ROOT}/predictions/postprocessed/{process_dict['run_id']}/polygons_{mode}_{name}.geojson", crs=f"EPSG:{crs}")
        ...
        df.to_csv(f"{ROOT}/predictions/postprocessed/{process_dict['run_id']}/comparison/seg_{name}.csv", index=False)
        
        df = mass_wasting.compare_mass_wasting_performance(ref_gdf, comp_gdf, (int(process_dict["year_end"]) - int(process_dict["year_start"])))
        if df is not None:
            df.to_csv(f"{ROOT}/predictions/postprocessed/{process_dict['run_id']}/comparison/dem_{name}.csv", index=False)
    return
